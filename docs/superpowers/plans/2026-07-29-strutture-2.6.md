...
- Suite: `python -m pytest -q` dalla radice. Parte da **61 test verdi**. I conteggi
  indicati negli step ("Expected: PASS, 86 test") sono **indicativi**: cio' che conta
  e' che nessun test fallisca e che quelli nuovi del task passino. Non aggiungere ne'
  togliere test per far quadrare un numero.
...
<div class="d-flex gap-2" id="azioni-struttura">
...
**Nessun pulsante Esporta o Elimina in questo task.** Le rispettive rotte non
esistono ancora, e un `url_for` verso una rotta assente fa fallire il rendering
della pagina. I Task 6 e 7 aggiungeranno il proprio pulsante dentro
`<div id="azioni-struttura">` insieme alla rotta che lo serve, cosi' ogni task
consegna qualcosa di completo e nessuno scrive codice destinato a essere
sostituito. La variabile `single_struttura` passata dalla rotta `scheda` serve al
Task 7: lasciala nel `render_template` anche se questo template non la usa
ancora.
...
In `strutture_bp.py`, dopo `riattiva`:
...
- [ ] **Step 5: Aggiungere il pulsante alla scheda**

In `templates/strutture/scheda.html`, dentro `<div class="d-flex gap-2" id="azioni-struttura">`,
**prima** del blocco `{% if not struttura.attiva %}`:
...
Run: `python -m pytest -q`
Expected: PASS, circa 90 test.

- [ ] **Step 7: Commit**

```bash
git add struttura_service.py strutture_bp.py templates/strutture/scheda.html tests/
...
In `strutture_bp.py`, dopo `esporta`:
...
- [ ] **Step 6: Aggiungere il pulsante alla scheda**

In `templates/strutture/scheda.html`, dentro `<div id="azioni-struttura">`, **dopo**
il form di riattivazione ma dentro lo stesso `{% if not struttura.attiva %}`, e con
il ramo `{% else %}` che spiega perche' il pulsante non c'e':
...
Il risultato finale del blocco deve essere: form Esporta (sempre), poi
`{% if not struttura.attiva %}` con Riattiva ed Elimina, `{% else %}` con la
spiegazione. `single_struttura` arriva gia' dalla rotta `scheda` (Task 5).

- [ ] **Step 7: Eseguire i test**

Run: `python -m pytest -q`
Expected: PASS, circa 96 test.

- [ ] **Step 8: Verificare la sensibilita' del freno sul nome**
