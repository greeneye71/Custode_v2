...
- Produces:
  ...
  - Fixture pytest `app`, `client` (il database temporaneo e' interno alla fixture `app`)
  - `pytest.ini` con `python_functions = test_*`: senza, pytest raccoglie come test
    anche `testo_sicuro` e l'helper `testo_di` e la suite esce con codice 1
...
- [ ] `python -m pytest tests/ -v` — 40 test verdi
